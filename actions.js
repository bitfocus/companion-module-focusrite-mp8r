module.exports = {
	...
	createAction: (instance, rcpCmd) => {
		...
		const mp8rChoices = require('./mp8rChoices.json')
		...
		if (rcpCmd.X > 1) {
			...
			if (mp8rChoices[actionName] !== undefined) {
				XOpts = {
					...XOpts,
					type: 'dropdown',
					label: mp8rChoices[actionName].xName || actionNameParts[rcpNameIdx],
					minChoicesForSearch: 0,
					choices: mp8rChoices[actionName].X,
					allowCustom: true,
				}
		...
		switch (rcpCmd.Type) {
			...
			case 'freq':
				if (rcpCmd.Max != 0 || rcpCmd.Min != 0) {
					if (mp8rChoices[actionName] !== undefined) {
						ValOpts.label = mp8rChoices[actionName].valName || actionNameParts[rcpNameIdx]
						ValOpts.choices = mp8rChoices[actionName].Val
						paramsToAdd.push(ValOpts)
					} else {
						ValOpts = {
							...ValOpts,
